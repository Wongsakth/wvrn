            ...
            <div className="flex flex-col gap-2">
                ...
                <div>
                  ...
                  <div style={{
                    ...
                    <div className="flex flex-col gap-2 pb-2">
                      {pastEvents.map(ev => (
                        <EventRow key={ev.id} event={ev} liked={likedIds.has(ev.id)} onLike={() => toggleLike(ev.id)} isPast />
                      ))}
                    </div>
                  </div>
              ...
              {upcomingEvents.length === 0 && pastEvents.length === 0
                ...
                : upcomingEvents.map(ev => (
                  <EventRow key={ev.id} event={ev} liked={likedIds.has(ev.id)} onLike={() => toggleLike(ev.id)} isPast={false} />
                ))
              }
              {upcomingEvents.length === 0 && pastEvents.length > 0 && (
                <EmptyState />
...
function EventRow({ event, liked, onLike, isPast }: any) {
  const start   = parseISO(event.start_date)
    ...
    >      <div style={{ width:isMulti?64:56, flexShrink:0, background:'var(--accent-muted)', borderRight:'1px solid var(--border)', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'8px 4px' }}>
      ...
      <div style={{ flex:1, minWidth:0, padding:'10px 12px' }}>
        <div style={{ display:'flex', gap:4, flexWrap:'wrap', marginBottom:4 }}>
          {isPast && (
            <span style={{ fontSize:10, padding:'1px 6px', borderRadius:20, background:'var(--surface-3)', color:'var(--text-muted)' }}>
              ผ่านไปแล้ว
            </span>
          )}
