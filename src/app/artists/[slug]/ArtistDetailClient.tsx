// app/artists/[slug]/ArtistDetailClient.tsx
'use client'
...
import {
  ...
  MapPin, Clock, ChevronRight, Music, ExternalLink, Flame, Loader2,
} from 'lucide-react'
...
export default function ArtistDetailClient({ artist, initialEvents }: Props) {
  ...
  // Check follow
  useEffect(() => {
    if (!user || !artist?.id) { setFollowed(false); return }
    sb.from('follows').select('artist_id')
      .eq('user_id', user.id).eq('artist_id', artist.id).maybeSingle()
  ...
  async function toggleFollow() {
    ...
    try {
      if (prev) {
        await sb.from('follows').delete().eq('user_id', user.id).eq('artist_id', artist.id)
        toast.success(`เลิกติดตาม ${artist.name}`)
      } else {
        await sb.from('follows').insert({ user_id: user.id, artist_id: artist.id })
  ...
  const bioTh = artist.bio_final_th
  const bioEn = artist.bio_final_en
          ...
          <div className="lg:col-span-1 flex flex-col gap-4">
            <div className="rounded-2xl overflow-hidden"
              ...
              <div className="h-24 flex items-end justify-center pb-0 relative"
                ...
                <div className="absolute bottom-0 translate-y-1/2 w-20 h-20 rounded-full overflow-hidden cursor-pointer"
                  style={{ border: '3px solid var(--surface-1)', background: 'var(--surface-2)' }}
                  onClick={() => artist.image_url && setLightbox(true)}>
                  {artist.image_url ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center">
                      <Music size={28} style={{ color: 'var(--accent)', opacity: 0.4 }} />
                    </div>
                  )}
                </div>
              </div>
            ...
            {(bioTh || artist.bio) && (
              <div className="rounded-xl p-4"
                ...
                <p className="text-[13px] text-secondary leading-relaxed">{bioTh || artist.bio}</p>
              </div>
            )}
                    ...
                    <a key={item.id} href={item.url} target="_blank" rel="noopener noreferrer"
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 text-[11px] text-muted">
                          ...
                          {item.published_at && (
                            <span>{format(parseISO(item.published_at), 'd MMM yyyy', { locale: th })}</span>
                          )}
                        </div>
                      </div>
