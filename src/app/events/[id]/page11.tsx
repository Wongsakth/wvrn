...
export default function EventDetailPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      setEvent({ ...data, artists: data.event_artists?.map((ea: any) => ea.artist) ?? [] })
      setLoading(false)
    }
    load()
  }, [id])
...
function ArtistRow({ artist }: { artist: any }) {
  return (
    <div className="flex items-center gap-3 p-2.5 rounded-xl transition-colors hover:bg-[var(--surface-2)]">
      {artist.image_url ? (
