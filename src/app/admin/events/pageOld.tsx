...
const EMPTY_FORM = {
  ...
  artist_orders: {} as Record<string, number>, // artist_id → sort_order
}
...
export default function EventsAdminPage() {
  ...
  async function load() {
    ...
    try {
      const [evRes, arRes, veRes] = await Promise.all([
        sb.from('events').select('*, venue:venues(name,province), event_artists(artist:artists(id,name))').order('start_date', { ascending: false }),
  ...
  function openEdit(ev: any) {
    ...
    ev.artists?.forEach((a: any, i: number) => { orders[a.id] = i + 1 })
    setForm({
      title:           ev.title,
      ...
      artist_ids:      ev.artists?.map((a: any) => a.id) ?? [],
                    ...
                    {form.artist_ids.map((aid, idx) => {
                      ...
                      return (
                        <div key={aid} className="flex items-center gap-2 py-1.5 px-2 rounded-lg"
                          ...
                          <span className="text-[12px] text-primary flex-1">{artist.name}</span>
                          <div className="flex gap-1">
                            <button onClick={() => moveArtist(aid, 'up')} disabled={idx === 0}
                              className="w-6 h-6 rounded flex items-center justify-center text-muted disabled:opacity-30 hover:text-primary"
